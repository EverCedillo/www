...
def OperationsGetRequest(messages, operation_id, project_id, zone):
  if hasattr(messages.ContainerProjectsZonesOperationsGetRequest, 'zone'):
    # v1beta2+:
    return messages.ContainerProjectsZonesOperationsGetRequest(
        operationId=operation_id, projectId=project_id, zone=zone)
  else:
    return messages.ContainerProjectsZonesOperationsGetRequest(
        operationId=operation_id, projectId=project_id, zoneId=zone)


def OperationsGetError(operation):
  if hasattr(operation, 'statusMessage'):
    # v1beta2+:
...
def WaitForOperation(operation, project_id, context, message,
  ...
  req = OperationsGetRequest(messages, operation_id=operation.name,
                             project_id=project_id, zone=operation.zone)
  ...
  if OperationsGetError(operation):
    raise Error('Operation [{0}] finished with error: {1}'.format(
        operation, OperationsGetError(operation)))
